...
1. Создаём файл конфигурации ```CMakePresets.json```, который будет ссылаться на файл цепочки ```toolchain``` (создаем его в одной директории с CMakeList.txt). Внутренности:
```
{
    "version": 3,
    "configurePresets": [
...
4. Важный шаг, который забывать не надо это группирование проекта (сконфигурация) 
   ```
   ...
   > Когда вы установите переменную окружения в переменное окружение Window всё равно есть вероятность ошибки выполнения команды сконфигурации, в таком случае рекомендуется в окне терминала повторно настроить  переменную окружения VCPKG_ROOT
